...
function filterKabupatenByProvince(provinsiId: number) {
  try {
    ...
    const hasilFilter = daftarKabupaten.filter(
      (item: any) => item.provinsi_id === provinsiId,
    );

    // 4. Menampilkan hasil
    if (hasilFilter.length > 0) {
      console.log(
        `Menampilkan ${hasilFilter.length} data untuk Provinsi ID: ${provinsiId}`,
      );
